...
function App() {
  ...
  const handleScanError = (error) => {
    ...
    setCameraError('Camera access denied. Please allow camera permissions or use manual entry below.');
  };
  ...
  const connectPrinter = async () => {
    try {
        ...
        setStatus('ERROR');
        addLog("✗ Connection failed: " + (e.response?.data?.message || "Printer unreachable"));
    }
  };

  // 3. Calculate Pages & Price
  ...
  const executePrint = async () => {
    ...
    try {
        await axios.post(`${API_URL}/api/print`, fd);
        setStatus('COMPLETED');
        addLog(`✓ Print job sent successfully`);
    } catch (e) {
        setStatus('PAYMENT');
        addLog("✗ Print failed: " + (e.response?.data?.error || e.message));
      ...
      <Card className="w-full max-w-md bg-slate-900/90 backdrop-blur-xl border-slate-700/50 shadow-2xl relative z-10">
        <CardHeader className="border-b border-slate-700/50 pb-4 bg-gradient-to-r from-slate-800/50 to-slate-900/50">
          <div className="flex justify-between items-center">
             ...
             <span className={`text-[10px] px-3 py-1.5 rounded-full font-bold border ${getStatusColor()}`}>
                {status}
             </span>
          </div>
        </CardHeader>
        ...
        <CardContent className="pt-6 space-y-6">
            ...
            {status === 'IDLE' && (
                <div className="space-y-4">
                    <div className="aspect-square bg-gradient-to-br from-slate-950 to-slate-900 rounded-2xl overflow-hidden border-2 border-slate-700/50 relative shadow-inner">
                        ...
                        />
                        <div className="absolute inset-0 border-4 border-blue-400/20 m-8 rounded-2xl animate-pulse pointer-events-none"></div>
                    ...
                    <details className="group">
                        ...
                        <div className="mt-3 space-y-2 p-3 bg-slate-800/30 rounded-lg border border-slate-700/30">
                            <input 
                                ...
                                className="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
                                onKeyPress={(e) => {
                ...
                <div className="space-y-4">
                    ...
                    {status === 'ERROR' && (
                        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-sm text-red-400">
                            <AlertCircle className="inline mr-2 h-4 w-4"/>
                            Connection failed. Check printer and try again.
                        </div>
                    )}
                    ...
                    <Button 
                        ...
                        className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white shadow-lg shadow-blue-500/20 font-semibold py-6"
                    >
                        {status === 'CONNECTING' ? (
                            <><Loader2 className="animate-spin mr-2 h-5 w-5"/> Connecting...</>
                        ) : (
                            <><Zap className="mr-2 h-5 w-5"/> Connect to Printer</>
                        )}
                    </Button>
            ...
            {status === 'PAYMENT' && pricing && (
                <div className="space-y-4">
                    ...
                    <Button 
                        ...
                        className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-bold py-6 text-lg shadow-lg shadow-emerald-500/20"
                    >
            ...
            {status === 'COMPLETED' && (
                <div className="space-y-6 text-center py-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Collect your document from the printer</p>
                    </div>
                    <Button 
                        onClick={resetFlow} 
            ...
            {/* Logs */}
            <div className="bg-black/50 backdrop-blur-sm text-emerald-400 text-[10px] font-mono p-4 rounded-xl h-28 overflow-y-auto border border-slate-800/50 shadow-inner scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                {logs.length === 0 ? (
